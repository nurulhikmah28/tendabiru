...
class Konfirmasi extends CI_Controller
{
	...
	function form()
	{
		...
		if (!$this->upload->do_upload('userfile')) {
			...
			echo $this->session->set_flashdata('message', '<div class="alert alert-info text-center">Terimakasih telah melakukan konfirmasi pembayaran.<br>
			Segera kami cek dan kami proses.</Terimakasih></div>');
			redirect('konfirmasi');
		}
	}
	...
	function tracking()
	{
		if (isset($_POST['submit1']) or $this->uri->segment(3) != '') {
			...
			if ($cek->num_rows() >= 1) {
				...
				$data['record'] = $this->db->query("SELECT a.kode_transaksi, b.*, c.nama_produk, c.satuan, c.produk_seo FROM `tb_toko_penjualan` a JOIN tb_toko_penjualandetail b ON a.id_penjualan=b.id_penjualan JOIN tb_toko_produk c ON b.id_produk=c.id_produk where a.kode_transaksi='" . $kode_transaksi . "'");
				$data['total'] = $this->db->query("SELECT a.kode_transaksi, a.resi, a.proses, sum((b.harga_jual*b.jumlah)) as total FROM `tb_toko_penjualan` a JOIN tb_toko_penjualandetail b ON a.id_penjualan=b.id_penjualan JOIN tb_toko_produk c ON b.id_produk=c.id_produk where a.kode_transaksi='" . $kode_transaksi . "'")->row_array();
